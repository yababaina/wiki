...
(function () {
  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      var isLocalGraph = depth >= 0;
      var removePages = (config.removePages || []).map(function (p) {
        return simplifySlug(p);
      });
      var removePagesSet = new Set(removePages);
      var highlightCurrentOnLoad =
        config.highlightCurrentOnLoad !== false && isLocalGraph;
      var includeFolder = config.includeFolder
        ? simplifySlug(config.includeFolder).replace(/\/$/, "")
        : null;

      function isInIncludedFolder(pageId) {
        if (!includeFolder) return false;
        return pageId === includeFolder || pageId.startsWith(includeFolder + "/");
      }

      function isRemovedPage(id) {
        if (!isLocalGraph) return false;
        if (removePagesSet.has(id)) return true;
        if (id.endsWith("/") && removePagesSet.has(id.slice(0, -1))) return true;
        if (removePagesSet.has(id + "/")) return true;
        return false;
      }
      ...
      if (isLocalGraph && removePagesSet.size > 0) {
        links = links.filter(function (link) {
          return !isRemovedPage(link.source) && !isRemovedPage(link.target);
        });
      }
      ...
      if (depth >= 0) {
        ...
        if (includeFolder && isInIncludedFolder(slug)) {
          validLinks.forEach(function (id) {
            if (
              isInIncludedFolder(id) &&
              !isRemovedPage(id) &&
              !id.startsWith("tags/")
            ) {
      ...
      function renderLabels() {
        ...
        for (var i = 0; i < nodeRenderData.length; i++) {
          ...
          } else if (hoveredNodeId !== null && nodeData.active) {
            nodeData.label.alpha = 0.9;
